...
class New_Predictor():
    def __init__(self):
        self.run_pass_model = joblib.load('run_pass_model.pkl')
        self.yds_gained_run_model = joblib.load('yds_gained_run_model.pkl')
        self.yds_gained_pass_model = joblib.load('yds_gained_pass_model.pkl')
        self.run_all_predictions = np.load('run_all_predictions.npy')
        self.run_actual_yards = np.load('run_actual_yards.npy')
        self.pass_all_predictions = np.load('pass_all_predictions.npy')
        self.pass_actual_yards = np.load('pass_actual_yards.npy')

    def predict_play_type(self, features):
        return self.run_pass_model.predict(features)[0]

    def predict_yards_gained(self, play_type, features):
        if play_type == 1: # Run
            model = self.yds_gained_run_model
            all_predictions = self.run_all_predictions
            actual_yards = self.run_actual_yards
        else: # Pass
            model = self.yds_gained_pass_model
            all_predictions = self.pass_all_predictions
            actual_yards = self.pass_actual_yards

        predicted_expected_yards = model.predict(features)[0]
...
def transform_exog_to_model(fit, exog):
    ...
    if transform and hasattr(self.model, 'formula') and exog is not None:
        ...
        exog = dmatrix(self.model.data.design_info, # removed .orig_exog
                       exog)
    if exog is not None:
...
class Basic_Predictor():
    def __init__(self, df, formula, logistic=False, method='newton'):
        self.df = df
    ...
    def train(self):
        ...
        if self.logistic:
            self.res = smf.logit(formula=self.formula, data=self.df).fit(disp=0, maxiter=250, method=self.method, skip_hessian=True) #ncg old
        else:
