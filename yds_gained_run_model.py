...
SELECT 
    ...
    case when down >= 3 and distance <= 2 then 1 else 0 end as obvious_run_situation,
...
# Offensive rushing average
off_avg_rush_yds = (
    runs_df.groupby("pos_team")["yards_gained"]
    .expanding()
    .mean()
    .reset_index(level=0, drop=True)
)
runs_df["temp_off_avg_yds"] = off_avg_rush_yds
runs_df["rolling_off_avg_rush_yards"] = runs_df.groupby("pos_team")[
    "temp_off_avg_yds"
].shift(1)

# Defensive rushing average allowed
def_avg_rush_yds_allowed = (
    runs_df.groupby("defense")["yards_gained"]
    .expanding()
    .mean()
    .reset_index(level=0, drop=True)
)
runs_df["temp_def_avg_yds"] = def_avg_rush_yds_allowed
runs_df["rolling_def_avg_rush_yards_allowed"] = runs_df.groupby("defense")[
    "temp_def_avg_yds"
].shift(1)
...
pre_game_off_stats = (
    runs_df.groupby(["game_id", "pos_team"])["rolling_off_avg_rush_yards"]
    .first()
    .reset_index()
)
pre_game_off_stats.rename(
    columns={"rolling_off_avg_rush_yards": "pre_game_off_avg_rush_yards"}, inplace=True
)

pre_game_def_stats = (
    runs_df.groupby(["game_id", "defense"])["rolling_def_avg_rush_yards_allowed"]
    .first()
    .reset_index()
)
pre_game_def_stats.rename(
    columns={
        "rolling_def_avg_rush_yards_allowed": "pre_game_def_avg_rush_yards_allowed"
    },
    inplace=True,
)
...
league_avg_rush_yds = df["yards_gained"].mean()
df["pre_game_off_avg_rush_yards"].fillna(league_avg_rush_yds, inplace=True)
df["pre_game_def_avg_rush_yards_allowed"].fillna(league_avg_rush_yds, inplace=True)

# --- Step 4: Create the matchup feature ---
df["pre_game_run_matchup"] = (
    df["pre_game_off_avg_rush_yards"] - df["pre_game_def_avg_rush_yards_allowed"]
)
...
features = [
    ...
    "pre_game_off_avg_rush_yards",
    "pre_game_def_avg_rush_yards_allowed",
    "pre_game_run_matchup",
    "obvious_run_situation",
    "previous_ppa",
    "ppa_change",
    "ret_usage",
    "ret_passing_usage",
...
joblib.dump(lgbm_yards, "yds_gained_run_model.pkl")
np.save("run_all_predictions.npy", all_predictions)
np.save("run_actual_yards.npy", actual_yards)
...
# --- USAGE EXAMPLE ---
row_number = 3
# Imagine in your simulation, it's a 1st & 10 run play
